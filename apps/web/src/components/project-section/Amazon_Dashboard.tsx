...
export default function AmazonDashboard({ processedData, className = "", style = {} as React.CSSProperties }: { processedData?: any; className?: string; style?: React.CSSProperties }) {
  ...
  }, [loadSample, raw.length, sampleLoaded]);
  ...
  // Helpers to group
  const by = (arr: any[], key: (x:any)=>string): Record<string, any[]> => arr.reduce<Record<string, any[]>>((acc, cur) => { const k = key(cur) || "(blank)"; (acc[k] ||= []).push(cur); return acc; }, {});
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {allFulfill.map(s => (
                  <span
                    key={s}
                    onClick={() => setFulfillmentSel((previous) => {
                      const next = new Set(previous);
                      if (next.has(s)) next.delete(s);
                      else next.add(s);
                      return next;
              ...
              <div className="overflow-y-auto rounded-xl p-2">
                <div className="flex flex-wrap gap-2">
                  {allCategories.map(c => (
                    <span
                      key={c}
                      onClick={() => setCategorySel((previous) => {
                        const next = new Set(previous);
