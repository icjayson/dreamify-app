...
import { useClerk, useUser, UserProfile } from "@/lib/clerk";
  ...
  return (
    <div className="w-full">
      <div className="px-2 sm:px-4 pt-8 pb-4 border-b border-border text-center">
        <h2 className="text-2xl md:text-3xl font-semibold text-foreground dark:text-white">Free Preview</h2>
        <p className="text-sm text-muted-foreground dark:text-white/70 mt-2">This invitation-only, non-commercial Hobby deployment has no checkout, subscription, or credit debit.</p>
      </div>
      <div className="p-4 md:p-6">
        <div className="mx-auto max-w-xl rounded-xl border border-primary/30 bg-gradient-to-b from-primary/15 to-primary/5 p-5 ring-1 ring-primary/20">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-foreground dark:text-white">Hobby demo profile</h3>
            <span className="px-2 py-0.5 text-[10px] rounded-full bg-primary/10 text-primary">CURRENT</span>
          </div>
          <div className="mt-3"><span className="text-4xl font-bold text-foreground dark:text-white">$0</span><span className="text-muted-foreground dark:text-white/70 text-sm"> hosting plan</span></div>
          <ul className="my-4 space-y-2 text-sm text-foreground/80 dark:text-white/80">
            <li>Deterministic demo AI without an LLM key</li>
            <li>File upload, chat, dashboards, and version history</li>
            <li>Technical daily usage and storage limits</li>
            <li>External connectors unavailable until certified</li>
          </ul>
          <button
            onClick={() => navigate(isSignedIn ? '/workspace' : '/login')}
            className="w-full button-gradient rounded-md py-2 text-sm"
          >
            {isSignedIn ? 'Open workspace' : 'Request access'}
          </button>
        </div>
      </div>
    </div>
...
export const PlansCreditsContent: React.FC = () => {
  ...
  return (
    <div className="w-full p-6">
      <div className="mb-1">
        <h2 className="text-2xl md:text-3xl font-semibold text-foreground dark:text-white">Preview limits</h2>
        <p className="text-sm text-muted-foreground dark:text-white/50 mt-1">Review the technical limits for this non-commercial deployment.</p>
      </div>

      {/* Top section: Profile and technical limits */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        ...
        <div className="col-span-1 w-full rounded-xl dark:border-white/10 border-border dark:bg-white/[0.03] bg-foreground/3 p-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-foreground dark:text-white font-semibold">Free Preview</p>
              <p className="text-xs text-muted-foreground dark:text-white/40">Invitation-only Hobby demo</p>
            </div>
          </div>
          <div className="space-y-0 text-sm text-foreground/70 dark:text-white/70">
            <div className="flex items-center gap-2">
              <Check className="w-3.5 h-3.5 text-primary" />
              <span>Up to {dailyDataRunLimit} data runs / day</span>
            </div>
            <div className="flex items-center gap-2">
              <Check className="w-3.5 h-3.5 text-primary" />
              <span>Deterministic demo provider</span>
            </div>
            <div className="flex items-center gap-2">
              <Check className="w-3.5 h-3.5 text-primary" />
              <span>Versioned dashboards</span>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-2 w-full rounded-xl dark:border-white/10 border-border dark:bg-white/[0.03] bg-foreground/3 p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-muted-foreground dark:text-white/70">Daily data-run cap</span>
            <span className="text-2xl font-bold text-foreground dark:text-white tabular-nums">Up to {dailyDataRunLimit.toLocaleString()}/day</span>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-foreground/80 dark:text-white/80">Server-enforced technical quota</p>
                <p className="text-[11px] text-muted-foreground dark:text-white/40">Live remaining usage is not exposed.</p>
              </div>
            </div>
          </div>
        </div>
...
const AccountCenterModal: React.FC<AccountCenterModalProps> = ({ open, activeTab, onChangeTab, onClose }) => {
  ...
  const sidebarItems = useMemo(() => {
    ...
    if (isSignedIn) {
      items.push(
        { key: "plans" as AccountCenterTab, label: "Preview limits", icon: <Sparkles className="w-4 h-4" /> },
