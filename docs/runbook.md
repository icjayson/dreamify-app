...
1. In Clerk Dashboard, set the application to invitation-only. The default
   ...
   custom `aud` claim.
2. Import the repository root (`.`) as one Vercel project and select the
   **Services** framework preset. The root `vercel.json` deploys `apps/web` as
   the `frontend` service and `services/api` as the `api` service.
   `services/morpheus-sandbox` is not an HTTP service; publish its command-only
   runner as a Vercel Sandbox snapshot and configure the resulting
   `SANDBOX_SNAPSHOT_ID` instead.
3. Confirm CI, secret scanning, contract drift, and Vercel builds are green.
4. Apply Alembic migrations with `DIRECT_DATABASE_URL` from a controlled environment.
5. Run the idempotent seed command.
6. Deploy the Services project once, then verify `/health`, `/api/v1/health`,
   the Next.js `/api/blob/*` gateway, and the end-to-end demo smoke test.
...
The API service must receive the corresponding exact CORS/Clerk origins, Blob
token, Blob gateway secret, internal service secret, workflow dispatch URL,
Postgres URLs, and runtime mode. In a Services project these variables are
configured on the shared Vercel project. A deploy that reports
`WEB_ENV_INVALID` is a configuration failure; correct the scoped Vercel
variables instead of bypassing the preflight.
