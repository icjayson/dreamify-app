...
import {
  integrationService,
  type ShopeeSellerConnectionStatusResponse,
  type ShopeeSellerReportResource,
  type ShopeeSellerShop,
...
const FALLBACK_REPORTS: ShopeeSellerReportResource[] = [
  ...
  { report_type: 'income', label: 'Income' },
];
...
export default function ShopeeSellerIntegrationModal() {
  const {
    isShopeeSellerModalOpen: isOpen,
    setShopeeSellerModalOpen: setOpen,
    currentProjectId,
    syncShopeeSeller,
    addFiles,
  } = useChatStore();
  ...
  const [connectionStatus, setConnectionStatus] = useState<ShopeeSellerConnectionStatusResponse | null>(null);
  const [reports, setReports] = useState<ShopeeSellerReportResource[]>(FALLBACK_REPORTS);
  const [shops, setShops] = useState<ShopeeSellerShop[]>([]);
  ...
  const loadResources = useCallback(async (status?: ShopeeSellerConnectionStatusResponse | null) => {
    setLoadingResources(true);
    try {
      const response = await integrationService.fetchShopeeSellerResources();
      if (!response.success) throw new Error(response.error || 'Failed to load Shopee Seller resources.');
      ...
      setError(err instanceof Error ? err.message : 'Failed to load Shopee Seller resources.');
    } finally {
      setLoadingResources(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onError = (msg: string) => {
      setConnecting(false);
      popupRef.current?.close();
      setError(msg || 'Shopee Seller authorization failed.');
    };

    const bc = new BroadcastChannel('shopee_seller_oauth');
    bc.onmessage = (e) => {
      if (e.data?.type === 'SHOPEE_SELLER_OAUTH_SUCCESS') onSuccess();
      else if (e.data?.type === 'SHOPEE_SELLER_OAUTH_ERROR') onError(e.data.error);
    };
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === 'SHOPEE_SELLER_OAUTH_SUCCESS') onSuccess();
      else if (event.data?.type === 'SHOPEE_SELLER_OAUTH_ERROR') onError(event.data.error);
    };
  ...
  const handleConnect = async () => {
    setConnecting(true);
    setError(null);
    const baseUrl = integrationService.getShopeeSellerOAuthStartUrl(region);
    const url = baseUrl;
    const width = 700;
    const height = 780;
  ...
  const handleDisconnect = async () => {
    ...
    try {
      await integrationService.disconnectShopeeSeller();
      setConnectionStatus(null);
  ...
  const handleSync = async () => {
    if (includePii) {
      setError('Shopee Seller v1 does not support buyer PII export.');
      return;
    }
    setSyncing(true);
    setError(null);
    try {
      ...
      const result = await syncShopeeSeller({
      ...
      addFiles([
        {
          ...
          sourceType: 'Shopee Seller',
          accountName: connectionStatus?.account_name || 'Shopee Seller',
    ...
    if (run.connectorKey && run.entityId) {
      try {
        if (existingProjectId) {
          ...
          const projectName = `${run.entityName || 'Shopee Seller'} Project`;
          const result = await integrationService.addConnectorEntityToNewProject(
            run.connectorKey,
            run.entityId,
            { project_name: projectName, prompt: 'Analyze this Shopee Seller marketplace data and build a social-commerce dashboard.', asset_id: run.asset_id }
          );
    ...
    addFiles([
      {
        ...
        filename: selectedAsset?.filename || run.asset_filename || 'shopee_seller.csv',
        size: selectedAsset?.size_bytes || run.config_snapshot?.size_bytes || 0,
        ext: selectedAsset?.extension || 'csv',
        status: 'uploaded',
        projectId: resolvedProjectId,
        sourceType: 'Shopee Seller',
        accountName: run.accountName || connectionStatus?.account_name || 'Shopee Seller',
        propertyName: run.entityName || 'Shopee Seller',
        ...
        <DialogHeader>
          <div className="mb-1 flex items-center gap-3">
            <div className="flex h-8 w-12 items-center justify-center overflow-hidden rounded bg-black">
              <img src="/shopee.png" alt="Shopee Seller" className="h-7 w-11 object-contain" />
            </div>
            <DialogTitle className="text-xl font-semibold">Connect Shopee Seller</DialogTitle>
          </div>
          <DialogDescription className="text-sm text-muted-foreground">
            Sync Shopee orders, products, inventory, returns, income, and marketplace revenue.
          </DialogDescription>
        </DialogHeader>
          ...
          {modalState === 'disconnected' && (
            <div className="space-y-4">
              <div className={modalStyles.infoPanel}>
                <p>Dreamify will request read-only Shopee Seller access.</p>
                <ul className="list-inside list-disc space-y-1 text-muted-foreground">
                  <li>Orders, order items, products, inventory, returns, and income</li>
            ...
            <>
              <div className="flex items-center justify-between rounded-lg border border-[#25F4EE]/40 bg-[#25F4EE]/10 p-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-xs text-muted-foreground">
                    {connectionStatus?.account_name || connectionStatus?.account_id || 'Shopee Seller'}
                  </p>
                </div>
              ...
              <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'new' | 'connected')} className="mt-4">
                ...
                <TabsContent value="connected" className="outline-none">
                  <ConnectedEntitiesList connectorKey="shopee_seller" onSelectAsset={handleSelectConnectedAsset} />
                </TabsContent>
              </Tabs>
            </>
          )}
