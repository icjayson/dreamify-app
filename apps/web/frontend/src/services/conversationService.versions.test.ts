...
describe("conversationService version history (Phase 7)", () => {
  ...
  it("revertDashboard posts the target version and returns the new head", async () => {
    ...
    expect(mockedApi.post).toHaveBeenCalledWith(
      ...
      { project_id: "proj_1", target_version: 2 },
    );
  });

  it("saveDashboardData omits the new fields when no options are passed", async () => {
    ...
    expect(mockedApi.put).toHaveBeenCalledWith(
      ...
      { project_id: "proj_1", dashboard_data: { foo: 1 } },
    );
  });
  ...
  it("saveDashboardData flags a 409 version conflict", async () => {
    mockedApi.put.mockResolvedValueOnce({ success: false, error: "HTTP error! status: 409" });
