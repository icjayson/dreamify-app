...
const MODAL_CONNECTORS = CONNECTORS.filter((connector) => connector.modalTarget);
...
export default function AllConnectorsModal() {
  ...
  const fetchStatuses = useCallback(async () => {
    const hasCertifiedConnector = MODAL_CONNECTORS.some(
  ...
  const handleConnectorClick = (connectorName: string, connectorKey?: string) => {
    if (!connectorCapability(capabilities, connectorKey).enabled) return;
    setOpen(false);
    setTimeout(() => {
      ...
      <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto p-0 gap-0">
        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-semibold">Connector catalog</DialogTitle>
              <p className="text-xs text-muted-foreground mt-0.5">
                File upload is guaranteed; optional connectors activate only after capability certification.
              </p>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="px-6 py-5 space-y-6">
          {CONNECTOR_CATEGORIES.map((category) => {
            const categoryConnectors = MODAL_CONNECTORS.filter((c) => c.category === category);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {categoryConnectors.map((connector) => {
                    ...
                    return (
                      <button
                        key={connector.name}
                        onClick={() => handleConnectorClick(connector.name, connector.connectorKey)}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {!availability.enabled ? (
                            <p className="text-xs text-muted-foreground mt-0.5 truncate">
                              {availability.reason ?? 'Unavailable in this deployment'}
                            </p>
                          ) : loading ? (
                        ...
                        {/* Right indicator */}
                        {!availability.enabled ? (
                          <span className="text-xs text-muted-foreground border border-border rounded-md px-2.5 py-1 flex-shrink-0">
                            Unavailable
                          </span>
                        ) : isConnected ? (
