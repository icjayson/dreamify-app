...
import { connectorCapability, useCapabilities } from "@/hooks/useCapabilities";
...
const WORKFLOW_STEPS = [
  { title: "Certify", description: "Verify credentials, callbacks, and provider access.", Icon: PlugZap },
  { title: "Enable", description: "Activate only after a provider smoke test succeeds.", Icon: RefreshCw },
...
const DATA_CONNECTORS_FAQ: { question: string; answer: string }[] = [
  {
    ...
    answer:
      "A data connector is an authenticated link between Dreamify and an external system. In this Hobby release, file upload is the only guaranteed source. The catalog preserves the migrated UI, but every external card fails closed until credentials exist and its OAuth or webhook checks and provider smoke test pass.",
  },
  {
    question: "Which data sources does Dreamify connect to?",
    answer:
      "CSV, XLSX, XLS, and flat JSON uploads are active. The catalog also shows planned or retained surfaces for advertising, analytics, commerce, CRM, messaging, spreadsheets, and warehouses; check each card's capability badge for deployed availability.",
  },
  {
    question: "Do I need to write SQL or build a data model?",
    answer:
      "No SQL is required for the guaranteed file-upload path. Upload a supported file and describe the dashboard in plain language; Dreamify profiles it and generates structured output within the demo limits.",
  },
  {
    question: "Is my data secure?",
    answer:
      "Uploads use private object storage, tenant-scoped paths, checksums, quotas, and short-lived access. External connector security is not assumed: a connector stays unavailable until its credential handling and provider-specific tests are certified.",
  },
  {
    question: "Can I combine multiple data sources in one dashboard?",
    answer:
      "A run can analyze up to three uploaded files within the 25 MiB total limit. Automatic joins for external live sources are not guaranteed in the Hobby release.",
  },
];

const PRODUCT_CONNECTORS = CONNECTORS.filter(
...
export default function ProductDataConnectorsPage() {
  ...
  const openConnectorModal = (target?: ConnectorModalTarget) => {
    const modalOpeners: Record<ConnectorModalTarget, () => void> = {
      ...
      zendesk: () => setZendeskModalOpen(true),
      mixpanel: () => setMixpanelModalOpen(true),
      posthog: () => setPostHogModalOpen(true),
      customer_io: () => setCustomerIOModalOpen(true),
      google_search_console: () => setGoogleSearchConsoleModalOpen(true),
  ...
  const handleConnectorClick = (connector: ConnectorItem) => {
    if (!connectorCapability(capabilities, connector.connectorKey).enabled) return;
    if (!isSignedIn) {
      navigate("/login");
      return;
    }
  ...
  return (
    <>
      <Seo
        title="Data Connectors — Dreamify | Capability-Gated Preview"
        description="Connect Meta Ads, Google Ads, GA4, TikTok Ads, AppsFlyer, Firebase, Google Search Console, Mixpanel, PostHog, Stripe, QuickBooks, Zendesk, Shopify, Klaviyo, Amazon Seller, TikTok Shop Seller, Shopee Seller, Lazada Seller, HubSpot, Salesforce, Pipedrive, Google Sheets, PostgreSQL, BigQuery, Snowflake, Databricks, and Supabase to Dreamify as each connector becomes available."
        canonical="https://dreamify-web.vercel.app/product/data-connectors"
        jsonLd={[
          {
            "@context": "https://schema.org",
            "@type": "CollectionPage",
            "@id": "https://dreamify-web.vercel.app/product/data-connectors#webpage",
            name: "Dreamify Data Connectors",
            url: "https://dreamify-web.vercel.app/product/data-connectors",
            description:
          ...
          },
          {
            ...
            "@type": "ItemList",
            name: "Dreamify Data Connectors",
            itemListElement: PRODUCT_CONNECTORS.map((c, i) => {
              ...
              return {
                ...
                name: c.name,
                url: hasSeoPage
                  ? `https://dreamify-web.vercel.app/product/data-connectors/${slug}`
                  : "https://dreamify-web.vercel.app/product/data-connectors",
              };
            }),
          },
          {
          ...
          <div className="mx-auto grid w-full max-w-6xl items-center gap-10 lg:grid-cols-[1.02fr_0.98fr]">
            <div className="text-center lg:text-left">
              ...
              <p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg lg:mx-0">
                File upload is available in this preview. External connector cards remain visible and activate only after credentials and a provider smoke test succeed.
              </p>
        ...
        <section ref={connectorsRef} className="relative px-5 py-12 sm:px-8" id="connectors">
          <div className="mx-auto max-w-6xl">
            <div className="mb-7 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h2 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Connector availability</h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Availability comes from the deployed capability policy. Uncertified connectors fail closed and cannot begin OAuth or sync.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {filteredConnectors.map((connector) => {
                ...
                const cardClassName = cn(
                  "group flex min-h-[92px] w-full items-center gap-4 rounded-lg border border-border bg-background/75 p-4 text-left shadow-sm backdrop-blur-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background",
                  availability.enabled
                    ? "hover:border-primary/45 hover:bg-background"
                  ...
                  <>
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className={cn(
                        ...
                        {availability.enabled ? "Active" : "Unavailable"}
                      </span>
                    </span>
                    <ChevronRight
                      aria-hidden="true"
                ...
                return (
                  <button
                    ...
                    title={availability.enabled ? undefined : availability.reason ?? "Connector unavailable"}
                    className={cardClassName}
                    aria-label={availability.enabled
                      ? `${isSignedIn ? "Connect" : "Log in to connect"} ${connector.name}`
