...
import {
  integrationService,
  type LazadaSellerConnectionStatusResponse,
  type LazadaSellerReportResource,
  type LazadaSellerAccount,
...
const FALLBACK_REPORTS: LazadaSellerReportResource[] = [
  ...
  { report_type: 'finance', label: 'Finance' },
];
...
export default function LazadaSellerIntegrationModal() {
  const {
    isLazadaSellerModalOpen: isOpen,
    setLazadaSellerModalOpen: setOpen,
    currentProjectId,
    syncLazadaSeller,
    addFiles,
  } = useChatStore();
  ...
  const [connectionStatus, setConnectionStatus] = useState<LazadaSellerConnectionStatusResponse | null>(null);
  const [reports, setReports] = useState<LazadaSellerReportResource[]>(FALLBACK_REPORTS);
  const [sellers, setSellers] = useState<LazadaSellerAccount[]>([]);
  ...
  const loadResources = useCallback(async (status?: LazadaSellerConnectionStatusResponse | null) => {
    setLoadingResources(true);
    try {
      const response = await integrationService.fetchLazadaSellerResources();
      if (!response.success) throw new Error(response.error || 'Failed to load Lazada Seller resources.');
      setReports(response.reports.length ? response.reports : FALLBACK_REPORTS);
      setSellers(response.sellers.length ? response.sellers : status?.sellers || []);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load Lazada Seller resources.');
    } finally {
      setLoadingResources(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onError = (msg: string) => {
      setConnecting(false);
      popupRef.current?.close();
      setError(msg || 'Lazada Seller authorization failed.');
    };

    const bc = new BroadcastChannel('lazada_seller_oauth');
    bc.onmessage = (e) => {
      if (e.data?.type === 'LAZADA_SELLER_OAUTH_SUCCESS') onSuccess();
      else if (e.data?.type === 'LAZADA_SELLER_OAUTH_ERROR') onError(e.data.error);
    };
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === 'LAZADA_SELLER_OAUTH_SUCCESS') onSuccess();
      else if (event.data?.type === 'LAZADA_SELLER_OAUTH_ERROR') onError(event.data.error);
    };
  ...
  const handleConnect = async () => {
    setConnecting(true);
    setError(null);
    const baseUrl = integrationService.getLazadaSellerOAuthStartUrl(region);
    const url = baseUrl;
    const width = 700;
    const height = 780;
  ...
  const handleDisconnect = async () => {
    ...
    try {
      await integrationService.disconnectLazadaSeller();
      setConnectionStatus(null);
  ...
  const handleSync = async () => {
    if (includePii) {
      setError('Lazada Seller v1 does not support buyer PII export.');
      return;
    }
    setSyncing(true);
    setError(null);
    try {
      ...
      const result = await syncLazadaSeller({
      ...
      addFiles([
        {
          ...
          sourceType: 'Lazada Seller',
          accountName: connectionStatus?.account_name || 'Lazada Seller',
    ...
    if (run.connectorKey && run.entityId) {
      try {
        if (existingProjectId) {
          ...
          const projectName = `${run.entityName || 'Lazada Seller'} Project`;
          const result = await integrationService.addConnectorEntityToNewProject(
            run.connectorKey,
            run.entityId,
            { project_name: projectName, prompt: 'Analyze this Lazada Seller marketplace data and build a social-commerce dashboard.', asset_id: run.asset_id }
          );
    ...
    addFiles([
      {
        ...
        filename: selectedAsset?.filename || run.asset_filename || 'lazada_seller.csv',
        size: selectedAsset?.size_bytes || run.config_snapshot?.size_bytes || 0,
        ext: selectedAsset?.extension || 'csv',
        status: 'uploaded',
        projectId: resolvedProjectId,
        sourceType: 'Lazada Seller',
        accountName: run.accountName || connectionStatus?.account_name || 'Lazada Seller',
        propertyName: run.entityName || 'Lazada Seller',
        ...
        <DialogHeader>
          <div className="mb-1 flex items-center gap-3">
            <div className="flex h-8 w-12 items-center justify-center overflow-hidden rounded bg-black">
              <img src="/lazada.png" alt="Lazada Seller" className="h-7 w-11 object-contain" />
            </div>
            <DialogTitle className="text-xl font-semibold">Connect Lazada Seller</DialogTitle>
          </div>
          <DialogDescription className="text-sm text-muted-foreground">
            Sync Lazada orders, products, inventory, returns, finance, and marketplace revenue.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-6">
          {modalState === 'checking' && (
            <div className={modalStyles.loadingCompact}>
              <Loader2 className="mb-2 h-6 w-6 animate-spin text-[#FF5A00]" />
          ...
          {modalState === 'disconnected' && (
            <div className="space-y-4">
              <div className={modalStyles.infoPanel}>
                <p>Dreamify will request read-only Lazada Seller access.</p>
                <ul className="list-inside list-disc space-y-1 text-muted-foreground">
                  <li>Orders, order items, products, inventory, returns, and finance</li>
                  <li>CSV-compatible assets with date, row, byte, seller, and region caps</li>
          ...
          {modalState === 'connected' && (
            <>
              <div className="flex items-center justify-between rounded-lg border border-[#FF5A00]/40 bg-[#FF5A00]/10 p-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-xs text-muted-foreground">
                    {connectionStatus?.account_name || connectionStatus?.account_id || 'Lazada Seller'}
                  </p>
                </div>
              ...
              <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'new' | 'connected')} className="mt-4">
                ...
                <TabsContent value="new" className="space-y-4 outline-none">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-2">
                      <Label className={modalStyles.label}>Seller</Label>
                      <Select value={sellerId} onValueChange={setSellerId} disabled={loadingResources}>
                        <SelectTrigger className={modalStyles.selectTrigger}>
                ...
                <TabsContent value="connected" className="outline-none">
                  <ConnectedEntitiesList connectorKey="lazada_seller" onSelectAsset={handleSelectConnectedAsset} />
                </TabsContent>
              </Tabs>
            </>
          )}
