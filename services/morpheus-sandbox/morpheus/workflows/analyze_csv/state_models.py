...
class UserState(BaseModel):
    ...
    conversation_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Nodes from S3 conversation"
    )
    user_assets: List[Dict[str, Any]] = Field(
        default_factory=list, description="Files/data available to user"
    )
    dashboards: Dict[str, Any] = Field(
        default_factory=dict, description="Existing dashboards in conversation"
    )
...
class WorkingMemory(BaseModel):
    ...
    dataframe_profile: Optional[Dict[str, Any]] = Field(
        default=None, description="DataFrame structure and statistics"
    )
    column_analysis: Optional[Dict[str, Any]] = Field(
        default=None, description="Per-column analysis results"
    )
    available_chart_types: Optional[str] = Field(
        default=None, description="Available chart types from knowledge base"
    )
    total_tokens: int = Field(
        default=0, description="Total tokens used in this workflow execution"
    )

    # Intermediate results
    current_query: Optional[str] = Field(
        default=None, description="Current query being processed"
    )
    python_execution_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="Results from Python REPL executions"
    )
    tool_outputs: Dict[str, Any] = Field(
        default_factory=dict, description="Outputs from all tool executions"
    )

    # Generation artifacts
    dashboard_json: Optional[Dict[str, Any]] = Field(
        default=None, description="Generated dashboard configuration"
    )
    dashboard_summary: Optional[str] = Field(
        default=None, description="Summary text explaining the generated dashboard"
    )
    chart_change_summary: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Structured what-changed summary from a chart modification (wired to BE/FE in a later phase)",
    )
    edit_provenance: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Data provenance behind a chart modification (wired to BE/FE in a later phase)",
    )
    analysis_steps: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Structured analysis steps (python/output/explanation) for activity transparency, for both generation and edits",
    )
    qa_response: Optional[str] = Field(
        default=None, description="Generated Q&A text response"
    )
    visual_artifacts: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Inline chart/table artifacts for Q&A visual responses",
    )

    # Error tracking
    errors: List[Dict[str, Any]] = Field(
        default_factory=list, description="Error history with context"
    )
    retry_count: int = Field(default=0, description="Number of retries attempted")
    rate_limit_hits: int = Field(
        default=0, description="Consecutive 429/rate-limit hits in reasoning nodes"
    )
...
class RouteDecision(BaseModel):
    ...
    next_step: Literal["dashboard", "qa", "qa_visual"] = Field(
        ..., description="The next workflow to run based on user intent."
    )
...
class AgentState(BaseModel):
    ...
    current_node: str = Field(
        default="START", description="Current node in state graph"
    )
    status: Literal["RUNNING", "FINISHED", "ERROR", "STOPPED"] = Field(
        default="RUNNING", description="Workflow execution status"
    )
    iteration: int = Field(default=0, description="Current iteration count")
    max_iterations: int = Field(
        default=60, description="Maximum iterations before timeout"
    )
    use_internal_reasoning: bool = Field(
        default=False,
        description="Use internal reasoning branch (single node replacing REASONING + EXECUTION + SYNTHESIS)",
    )

    # Input/Output
    input_prompt: str = Field(description="User's request/question")
    file_paths: List[str] = Field(
        default_factory=list, description="Paths to CSV/data files being analyzed"
    )
    assets_dict: Dict[str, str] = Field(
        default_factory=dict,
        description="Dictionary mapping filenames to their local temp paths",
    )
    data_profile: Optional[str] = Field(
        default=None, description="Output of the exploration step"
    )
    ...
    chart_mentions: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Charts referenced by user for targeted modification",
    )

    # Template guidance
    template_spec: Optional[Dict[str, Any]] = Field(
        default=None, description="Active template content spec for guided generation"
    )
    theme_id: Optional[str] = Field(
        default=None, description="Selected visual theme for dashboard styling"
    )
    ...
    conversation_uri: Optional[str] = Field(
        default=None, description="S3 URI for conversation persistence"
    )
    conversation_backup_uri: Optional[str] = Field(
        default=None, description="Backup S3 URI"
    )
...
class ActionRequest(BaseModel):
    ...
    tool_name: Optional[str] = Field(
        default=None,
        description="Name of tool to execute (if action_type is EXECUTE_TOOL)",
    )
    arguments: Dict[str, Any] = Field(
        default_factory=dict, description="Arguments for the action/tool"
    )
...
class ActionResult(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(
        default=None, description="Action output data"
    )
    error: Optional[str] = Field(default=None, description="Error message if failed")
    next_node: Optional[str] = Field(
        default=None, description="Suggested next node (optional)"
    )
