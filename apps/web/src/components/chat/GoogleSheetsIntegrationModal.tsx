...
export default function GoogleSheetsIntegrationModal() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (!oauthToken && !selectedFileName && !loading && !tokenLoadAttemptedRef.current) {
        tokenLoadAttemptedRef.current = true;
        void loadToken();
      }
    } else {
      tokenLoadAttemptedRef.current = false;
      setOAuthToken(null);
      setError(null);
  ...
  }, [clearAuthError, hasScopes, isGoogleLinked, isOpen, loading, oauthToken, selectedFileName]);
  ...
  const handleOpenPicker = () => {
    ...
    try {
      ...
      const developerKey = process.env.NEXT_PUBLIC_GOOGLE_PICKER_API_KEY as string;
