...
import { SignedIn, useAuth } from "@/lib/clerk";
...
const HomePage = ({ onGetStarted, onProcessedDataChange }: HomePageProps) => {
  ...
  useEffect(() => {
    const currentWord = ROTATING_WORDS[rotatingIndex].text;
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (files.length > remainingSlots) {
      toast({
        ...
        description: `Maximum ${maxFilesPerRun} files allowed. You can add ${Math.max(remainingSlots, 0)} more file(s).`,
        variant: "destructive"
      });
      return;
    }
  ...
  const handleIntegrationClick = (connector: ConnectorItem) => {
    ...
    if (connector.name === 'Zendesk') {
      setDropdownOpen(false);
      setTimeout(() => useChatStore.getState().setZendeskModalOpen(true), 0);
      return;
    }
    if (connector.name === 'Mixpanel') {
      setDropdownOpen(false);
      setTimeout(() => useChatStore.getState().setMixpanelModalOpen(true), 0);
      return;
    }
              ...
              <h2 className="text-xl lg:text-3xl text-foreground dark:text-white font-inter">
                ...
                >
                  <span
                    ...
                    style={{ backgroundImage: ROTATING_WORDS[rotatingIndex].gradient }}
                  >
                    {displayedText || '\u00A0'}
                  </span>
                  <span
                    style={{
                      ...
                      width: '2px',
                      height: '0.75em',
                      backgroundColor: ROTATING_WORDS[rotatingIndex].cursorColor,
                      marginLeft: '1px',
            ...
            >
              ...
              <div className="relative mb-4">
                <TextareaAutosize
                  ...
                  placeholder={isListening ? 'Listening...' : CHAT_PLACEHOLDERS[placeholderIndex]}
                  ...
                  <div className="relative data-source-dropdown">
                    ...
                    {dropdownOpen && (
                      <div className="absolute bottom-full left-0 mb-1 bg-background/95 backdrop-blur-sm border border-border rounded-xl shadow-2xl z-10 p-2 w-52">
                        ...
                        {/* Capability-gated connector catalog */}
                        {CONNECTORS.filter(c => ['GA4', 'Google Ads', 'Firebase', 'Google Sheets'].includes(c.name)).map(con => {
                          ...
                          return (
                            <button
                              ...
                              title={availability.enabled ? `Connect ${con.name}` : availability.reason || "Unavailable in the Hobby demo"}
                              className="w-full px-2 py-1.5 text-left text-sm flex items-center gap-2 hover:bg-muted rounded-md transition-colors disabled:cursor-not-allowed disabled:opacity-60"
                            >
                              <img src={con.icon} alt="" className="w-4 h-4 object-contain flex-shrink-0" />
                        ...
                        {/* Additional unavailable connector previews */}
                        {([
                          ...
                          <button
                            key={con.name}
                            disabled
                            title="Unavailable in the Hobby demo"
                            className="w-full px-2 py-1.5 text-left text-sm flex items-center gap-2 rounded-md transition-colors cursor-not-allowed opacity-60"
                          >
            ...
            <div className="flex flex-col gap-2 mt-4 sm:mt-6 animate-fade-in w-full" style={{ animationDelay: '0s' }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {[
                  ...
                  { icon: LayoutDashboard, text: "Build a comprehensive dashboard from my uploaded data." }
                ].map((item, index) => (
                  <button
                    key={index}
