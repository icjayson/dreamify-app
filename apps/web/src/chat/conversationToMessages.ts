...
type MessageAttachmentFiles = NonNullable<NonNullable<Message['attachment']>['files']>;
  ...
  const restoredMessages: Message[] = nodes
    ...
    .map((node, index, array) => {
      ...
      if (Array.isArray(visualArtifactsContent?.data?.artifacts)) {
        const artifacts = compact(visualArtifactsContent.data.artifacts
      ...
      if (assetContent?.data) {
        ...
        const filesFromNodeContents = assetContents
          ...
          .filter(Boolean) as MessageAttachmentFiles;
