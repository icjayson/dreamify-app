...
import { useUser, useSession, useSessionList } from "@/lib/clerk";
import { Button } from "@/components/ui/button";
import { Monitor, Smartphone, Tablet } from "lucide-react";
import { formatToDisplay, getNow } from "@/utils/timestamp";
import { ByokProviderSettings } from "@/components/homepage-section/ByokProviderSettings";
...
const AccountSettings: React.FC = () => {
  ...
  const disconnectExternalAccount = async (provider: string, id: string, destroy: () => Promise<unknown>) => {
  ...
  const setPrimaryEmail = async (emailId: string) => {
    ...
    try {
      ...
      // Clerk remains authoritative; keep the current email selected on failure.
    }
  };
  ...
  const removeEmail = async (emailId: string) => {
    ...
    try {
      ...
      // Clerk remains authoritative; leave the address visible on failure.
    }
  };
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border border-border dark:border-white/10 bg-muted/50 dark:bg-black/30 p-3">
                ...
                <div className="text-sm text-muted-foreground dark:text-white/80 mt-1">
                  IP address and location are not collected by this page
                </div>
