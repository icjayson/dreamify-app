...
SHOPEE_PROVIDER = "shopee_seller"
SHOPEE_ASSET_TYPE = "integration_shopee_seller"
DEFAULT_API_BASE_URL = "https://partner.shopeemobile.com"
AUTHORIZATION_PATH = "/api/v2/shop/auth_partner"
...
REPORT_ENDPOINTS = {
    "sales_overview": "/api/v2/order/get_order_list",
    "orders": "/api/v2/order/get_order_list",
    "order_items": "/api/v2/order/get_order_detail",
    "products": "/api/v2/product/get_item_list",
    "inventory": "/api/v2/product/get_item_list",
    "returns": "/api/v2/returns/get_return_list",
    "income": "/api/v2/payment/get_escrow_detail",
}
...
def _normalize_region(value: Any) -> str:
    ...
    if region not in SUPPORTED_REGIONS:
        raise HTTPException(status_code=400, detail="Unsupported Shopee Seller region")
    return region
...
class ShopeeSellerAdapter:
    ...
    async def api_request(
        ...
        *,
        ...
        shops: Optional[Sequence[Dict[str, Any]]] = None,
        ...
        query_params = dict(params or {})
        selected_shop_id = str(query_params.pop("shop_id", "") or shop_id or "")
        body = (
        ...
        headers = {
            ...
            "User-Agent": "DreamifyShopeeSellerConnector/1.0",
        }
        if body:
    ...
    async def fetch_shops(self, ctx: Dict[str, Any]) -> List[Dict[str, Any]]:
        known_shops = ctx.get("shops") if isinstance(ctx.get("shops"), list) else []
        target_shops = known_shops or [{"id": ctx.get("shop_id"), "region": ctx.get("region")}]
        ...
        for shop in target_shops:
            ...
            try:
                payload = await self.api_request(
                    ...
                    params={"shop_id": str(shop.get("id"))},
                    **ctx,
                )
                ...
                enriched_shop["name"] = (
                    data.get("shop_name")
                    or data.get("name")
                    or enriched_shop.get("name")
                    or "Shopee Seller"
                )
            except Exception as exc:
                logger.info("Shopee Seller shop info lookup failed: %s", exc)
            enriched.append(self._flatten_shop(enriched_shop, ctx.get("region")))
        if enriched:
            return [shop for shop in enriched if shop.get("id")]
        try:
            payload = await self.api_request(
                method="GET",
                path="/api/v2/shop/get_shop_info",
            ...
            data = payload.get("response") or payload.get("data") or {}
            raw_shops = data.get("shops") or data.get("shop_list") or []
            if not raw_shops and data:
                raw_shops = [data]
            shops = [self._flatten_shop(item, ctx.get("region")) for item in raw_shops]
            return [shop for shop in shops if shop.get("id")]
        except Exception as exc:
            logger.info("Shopee Seller authorized shop lookup failed: %s", exc)
            return []
    ...
    async def fetch_report_rows(
        ...
        *,
        ...
        endpoints: List[str] = []
        target_shops = list(shops) or [{"id": "all", "region": ctx.get("region") or "VN"}]
        for shop in target_shops:
    ...
    def _request_body(
        ...
        if report_type in {"products", "inventory"}:
            return {"page_size": page_size, "offset": 0}
        if report_type == "income":
            return {"page_size": page_size, "order_sn": ""}
        return {
            "page_size": page_size,
    ...
    def _extract_rows(self, report_type: str, data: Dict[str, Any]) -> List[Any]:
        candidates = {
            ...
            "income": ("order_income", "income", "escrow_detail", "records", "items"),
        }[report_type]
    ...
    def _flatten_shop(self, item: Dict[str, Any], fallback_region: Any) -> Dict[str, Any]:
        return {
            ...
            "name": item.get("shop_name") or item.get("name") or "Shopee Seller",
    ...
    def _flatten_report_row(
        ...
        return {
            "income_id": _lookup(item, "income_id", "escrow_id", "id"),
            "order_id": _lookup(item, "order_id", "order_sn"),
            "shop_id": shop.get("id"),
            "currency": _lookup(item, "currency"),
            "revenue": _lookup(item, "revenue", "gross_revenue", "order_income"),
            "fees": _lookup(item, "fees", "platform_fees", "commission_fee"),
            "refund_amount": _lookup(item, "refund_amount", "buyer_refund_amount"),
            "net_amount": _lookup(item, "net_amount", "escrow_amount"),
            "income_time": _lookup(item, "income_time", "settlement_time", "create_time"),
...
class ShopeeSellerConnectorService:
    def __init__(self, adapter: Optional[ShopeeSellerAdapter] = None):
        self.adapter = adapter or ShopeeSellerAdapter()

    def _shopee_config(self) -> Dict[str, str]:
        cfg = getattr(config, "shopee_seller", None)
        return {
            "partner_id": (getattr(cfg, "partner_id", "") if cfg else "")
            or os.environ.get("SHOPEE_PARTNER_ID", ""),
            "partner_key": (getattr(cfg, "partner_key", "") if cfg else "")
            or os.environ.get("SHOPEE_PARTNER_KEY", ""),
            "redirect_uri": (getattr(cfg, "redirect_uri", "") if cfg else "")
            or os.environ.get("SHOPEE_REDIRECT_URI", ""),
            "api_base_url": (getattr(cfg, "api_base_url", "") if cfg else "")
            or os.environ.get("SHOPEE_API_BASE_URL", DEFAULT_API_BASE_URL),
            "default_region": (getattr(cfg, "default_region", "") if cfg else "")
            or os.environ.get("SHOPEE_DEFAULT_REGION", "VN"),
        }

    def _make_state_payload(self, user_id: str, region: str) -> str:
        cfg = self._shopee_config()
        secret = cfg["partner_key"] or config.app.secret_key
        payload = {
            "u": user_id,
            "r": region,
    ...
    def _save_metadata(self, user_id: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        existing = (
            connected_accounts_repo.get_connection(user_id, SHOPEE_PROVIDER) or {}
        )
        preserved = {
            key: value
    ...
    def get_oauth_url(self, user_id: str, region: str = "VN") -> str:
        ...
        if not cfg["partner_id"] or not cfg["partner_key"]:
            raise ValueError("Shopee Seller partner credentials are not configured.")
        ...
        if cfg["redirect_uri"]:
            params["redirect"] = cfg["redirect_uri"]
        url = self.adapter._signed_url(
    ...
    async def handle_oauth_callback(
        self, code: str, state: str, shop_id: Optional[str] = None
    ) -> None:
        cfg = self._shopee_config()
        if not cfg["partner_id"] or not cfg["partner_key"]:
            raise ValueError("Shopee Seller OAuth credentials are not configured.")
        payload = self._verify_state(state)
        user_id = str(payload["u"])
        region = _normalize_region(payload.get("r"))
        selected_shop_id = str(shop_id or "").strip()
        if not selected_shop_id:
            raise HTTPException(
                status_code=400,
                detail="Shopee Seller OAuth callback did not include shop_id.",
            )
        record = (
            connected_accounts_repo.get_connection(user_id, SHOPEE_PROVIDER) or {}
        )
        ...
        if not pending.pop(state, None):
            raise ValueError("Missing Shopee Seller OAuth state.")
        ...
        if not refresh_token:
            raise HTTPException(
                status_code=400,
                detail="Shopee Seller OAuth response did not include a refresh token.",
            )
        expires_at = self._expires_at(token_data, "expire_in", 3600)
        refresh_expires_at = self._expires_at(
            token_data, "refresh_token_expire_in", 365 * 24 * 3600
        )
        shop = {
            "id": selected_shop_id,
            "name": record.get("account_name") or f"Shopee Shop {selected_shop_id}",
            "region": region,
        }
        metadata = self._save_metadata(
            user_id,
            {
                ...
                "account_id": token_data.get("merchant_id")
                or token_data.get("main_account_id")
                or selected_shop_id,
                "account_name": record.get("account_name") or "Shopee Seller",
                "region": region,
                "api_base_url": cfg["api_base_url"],
                "shops": [shop],
        ...
        try:
            ...
            logger.info("Shopee Seller post-connect shop lookup failed: %s", exc)
    ...
    async def _api_context(
        ...
        shops = current.get("shops") if isinstance(current.get("shops"), list) else []
        primary_shop_id = str((shops[0] or {}).get("id") or "") if shops else ""
        return {
    ...
    async def _access_token(
        ...
        if not encrypted_refresh:
            raise HTTPException(status_code=401, detail="Shopee Seller is not connected.")
    ...
    async def _refresh_access_token(
        ...
        if not encrypted_refresh:
            raise HTTPException(
                status_code=401,
                detail="Shopee Seller refresh token is missing. Please reconnect.",
            )
        shops = record.get("shops") if isinstance(record.get("shops"), list) else []
        shop_id = str((shops[0] or {}).get("id") or "") if shops else ""
        ...
        access_token = str(token_data.get("access_token") or "")
        if not access_token:
            raise HTTPException(status_code=401, detail="Shopee Seller token refresh failed.")
    ...
    async def disconnect(self, user_id: str) -> None:
        connected_accounts_repo.delete_connection(user_id, SHOPEE_PROVIDER)
    ...
    def parse_entity_id(self, entity_id: str) -> Dict[str, str]:
        ...
        if len(parts) != 4 or parts[0] != "shopee_seller":
            raise HTTPException(status_code=400, detail="Invalid Shopee Seller entity id")
        report_type = parts[1]
        if report_type not in VALID_REPORT_TYPES:
            raise HTTPException(status_code=400, detail="Invalid Shopee Seller report type")
        return {
            "report_type": report_type,
    ...
    def _selected_entity(
        ...
        account_name = str(record.get("account_name") or "Shopee Seller")
        label = REPORT_LABELS.get(report_type, report_type.replace("_", " ").title())
        return {
            "id": f"shopee_seller:{report_type}:{shop_id or 'all'}:{region}",
    ...
    async def sync(
        ...
        if report_type not in VALID_REPORT_TYPES:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "products, inventory, returns, or income."
                ),
            )
        if include_pii:
            raise HTTPException(
                status_code=400,
                detail="Shopee Seller v1 does not support buyer PII export.",
            )
        ...
        if len(csv_content) > byte_cap:
            raise HTTPException(
                status_code=413,
                detail="Shopee Seller extract exceeded the configured byte cap.",
            )
        return self._save_shopee_asset(
            user_id=user_id,
            project_id=project_id,
            record=record,
            report_type=report_type,
    ...
    def _resolve_shops(
        ...
        if isinstance(shops, list):
            normalized = [
                {
                    ...
                    "name": item.get("name") or item.get("shop_name") or "Shopee Seller",
            ...
            if shop_id and shop_id != "all":
                ...
                if matched:
                    return matched
                raise HTTPException(status_code=400, detail="Unknown Shopee Seller shop_id.")
            if normalized:
                return normalized
        return [{"id": shop_id or "all", "name": "Shopee Seller", "region": region}]

    def _save_shopee_asset(
        self,
        user_id: str,
        project_id: str,
        record: Dict[str, Any],
        ...
        filename = (
            f"shopee_seller_{_sanitize_filename_part(report_type)}_"
        ...
        connected_accounts_repo.append_selected_entity(
            user_id=user_id, provider=SHOPEE_PROVIDER, entity=entity
        )
        updated = assets_repo.update_asset_metadata(
            ...
            metadata={
                ...
                "shopee_report_type": report_type,
                "shopee_region": region,
                "shopee_manifest_s3_key": manifest_key,
                "shopee_manifest": manifest,
            },
        )
        return {
            "success": True,
            "message": (
                f"Successfully synced {len(rows)} rows from Shopee Seller "
                f"({entity['name']})."
            ),
...
shopee_seller_service = ShopeeSellerConnectorService()
