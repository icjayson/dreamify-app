...
class ConversationService {
  ...
  private async currentDashboardVersion(
    conversationId: string,
    dashboardId: string,
    projectId: string,
  ): Promise<number> {
    const versions = await this.getDashboardVersions(conversationId, dashboardId, projectId);
  ...
  async updateDashboardTemplate(
    ...
    try {
      const expectedVersion = await this.currentDashboardVersion(conversationId, dashboardId, projectId);
      await api.put(
        `/api/v1/conversation/${conversationId}/dashboard/${dashboardId}/template`,
        { project_id: projectId, template_id: templateId, expected_version: expectedVersion },
      );
    } catch (error) {
  ...
  async updateDashboardTheme(
    ...
    try {
      ...
      await api.put(
        ...
        { project_id: projectId, theme_id: themeId, expected_version: expectedVersion },
      );
    } catch (error) {
  ...
  async saveDashboardData(
    ...
    try {
      const expectedVersion = options?.expectedVersion ?? await this.currentDashboardVersion(
        conversationId,
        dashboardId,
        projectId,
      );
  ...
  async revertDashboard(
    ...
    const expectedVersion = await this.currentDashboardVersion(
      conversationId,
      dashboardId,
      projectId,
    );
    const response = await api.post<DashboardRevertResponse>(
      ...
      { project_id: projectId, target_version: targetVersion, expected_version: expectedVersion },
    );
    if (response.success && response.data) {
      return response.data;
    }
