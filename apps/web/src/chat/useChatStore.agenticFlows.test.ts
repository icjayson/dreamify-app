...
type TestNodeContent = { type: string; data: Record<string, unknown> };

function userNode(node_id: string, text: string, extraContents: TestNodeContent[] = []) {
  return {
    node_id,
    role: "user",
...
function assistantTextNode(node_id: string, text: string, extraContents: TestNodeContent[] = []) {
  return {
    node_id,
    role: "assistant",
  ...
  it("generates a dashboard from a fresh upload and opens the returned dashboard card", async () => {
    ...
    mockedFileService.getAsset.mockResolvedValue({
      ...
      asset: {
        ...
        storage_ref: { provider: "vercel_blob", pathname: "uploads/file_1.csv" },
        extension: "csv",
        filename: "sales.csv",
        status: "uploaded",
        size_bytes: 2048,
  ...
  it("keeps a pre-conversation template through pending-action navigation reset", async () => {
    ...
    mockedFileService.getAsset.mockResolvedValue({
      ...
      asset: {
        ...
        storage_ref: { provider: "vercel_blob", pathname: "uploads/file_template.csv" },
        extension: "csv",
        filename: "growth.csv",
        status: "uploaded",
        size_bytes: 4096,
