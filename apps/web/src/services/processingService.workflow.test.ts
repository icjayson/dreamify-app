...
describe("processingService.runProcessing request contracts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockedConversationService.sendChatMessage.mockResolvedValue({
      conversation_id: "conversation_1",
  ...
  it("sends a stable request id and explicit dashboard/component edit target", async () => {
    const contents = [
      {
        type: "chart_mention",
        data: {
    ...
    await processingService.runProcessing(
      "project_1",
      null,
    ...
    const requests = mockedConversationService.sendChatMessage.mock.calls.map(([request]) => request);
    expect(requests.map((request) => request.client_request_id)).toEqual([
      "submission-0001",
      "submission-0001",
    ]);
    expect(requests[0]?.edit_target).toEqual({
      dashboard_id: "dashboard-a",
      component_ids: ["component-revenue", "chart-revenue"],
    });
  });

  it("creates a new id for each new logical submission", async () => {
    await processingService.runProcessing("project_1", null, "First");
    await processingService.runProcessing("project_1", null, "Second");

    const ids = mockedConversationService.sendChatMessage.mock.calls.map(
      ([request]) => request.client_request_id,
    );
